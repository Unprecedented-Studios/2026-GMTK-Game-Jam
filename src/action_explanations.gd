...
func _ready() -> void:
	...
	basic_attack.set_action_display(Action.actions_list.BasicAttack)
	$ActionList.add_child(basic_attack)
	var small_heal:ActionDisplay = action_display_preload.instantiate()
	small_heal.set_action_display(Action.actions_list.SmallHeal)
...
func add_explanation(new_explanation:Action.actions_list):
